...
test('paint session controller enforces 20-step undo/redo, dirty flag tracking, and name validation', () => {
  ...
  assert.match(paintViewJs, /customArtRepo\.saveArtwork/);
  assert.match(paintViewJs, /customArtRepo\.computeSha256/);
  assert.match(paintViewJs, /customArtRepo\.saveDraft\(blob/);
  assert.match(paintViewJs, /type: 'customAsset\/add',[\s\S]*asset: customMetadata/);
});
...
test('paint studio keyboard shortcuts guard against browser modifier keys and support draft flushing', () => {
  ...
  assert.match(paintViewJs, /flushDraftCheckpoint/);
